...
import { discreteTrialForwardChainResultInitialFixture } from "../fixtures/result.fixtures";
...
describe("Discrete Trial Forward Chaining Result Page Tests", () => {
  ...
  test("Should render with a discrete trial forward chaining result", async () => {
  ...
  test("Should add/remove a discrete trial forward chaining result", async () => {
    const id = mockDbService.getUuidByKey(
      createTargetUuidKey(Domains.Program, 1)
    );
    const { app } = await makeTestApp({ initialRoute: `/results/${id}` });
