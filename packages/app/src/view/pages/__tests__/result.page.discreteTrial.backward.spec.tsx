...
import { discreteTrialBackwardChainResultInitialFixture } from "../fixtures/result.fixtures";
...
describe("Discrete Trial Backward Chaining Result Page Tests", () => {
  const mockDbService = Container.get(MockDBService);
  beforeEach(async () => {
    await mockDbService.cleanUp();
    await mockDbService.setUpData(
      discreteTrialBackwardChainResultInitialFixture
    );
  });
  ...
  test("Should render with a discrete trial backward chaining result", async () => {
  ...
  test("Should add/remove a discrete trial backward chaining result", async () => {
    const id = mockDbService.getUuidByKey(
      createTargetUuidKey(Domains.Program, 1)
    );
    const { app } = await makeTestApp({ initialRoute: `/results/${id}` });
