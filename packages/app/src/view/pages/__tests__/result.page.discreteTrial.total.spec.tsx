...
import { discreteTrialResultInitialFixture } from "../fixtures/result.fixtures";
...
describe("Discrete Trial Total Chaining Results Page Tests", () => {
  const mockDbService = Container.get(MockDBService);
  beforeEach(async () => {
    await mockDbService.cleanUp();
    await mockDbService.setUpData(discreteTrialResultInitialFixture);
  });
  ...
  test("Should render with a discrete trial total chaining result", async () => {
  ...
  test("Should add/remove a discrete trial total chaining result", async () => {
    const id = mockDbService.getUuidByKey(
      createTargetUuidKey(Domains.Program, 1)
    );
    const { app } = await makeTestApp({ initialRoute: `/results/${id}` });
